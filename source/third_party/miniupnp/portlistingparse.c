...
data(void * d, const char * data, int l)
{
	...
	case NewExternalPort:
		pm->externalPort = (unsigned short)__atoui(data, l);
		break;
	case NewProtocol:
		if(l > 3)
			l = 3;
	...
	case NewInternalPort:
		pm->internalPort = (unsigned short)__atoui(data, l);
		break;
	case NewInternalClient:
		...
		break;
	case NewEnabled:
		pm->enabled = (unsigned char)__atoui(data, l);
		break;
	case NewDescription:
